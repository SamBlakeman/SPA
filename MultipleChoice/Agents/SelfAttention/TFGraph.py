...
class TFGraph(object):
    ...
    def __init__(self, directory, vgg16_npy_path, input_dim, rand_action_prob, num_actions, attention_dim):
        self.ti = 0
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('value'):
                self.hidden1 = tf.stop_gradient(self.pool5_flat)

                self.keys = tf.layers.dense(inputs=self.hidden1, units=self.attention_dim, activation=tf.nn.relu)
                self.queries = tf.layers.dense(inputs=self.hidden1, units=self.attention_dim, activation=tf.nn.relu)
                self.values = tf.layers.dense(inputs=self.hidden1, units=self.attention_dim, activation=tf.nn.relu)
                ...
                self.attention = tf.nn.softmax(self.attention_scaled, dim=-1)
                self.attention_output = tf.matmul(self.attention, self.values)

                self.hidden2 = tf.layers.dense(inputs=self.attention_output, units=512, activation=tf.nn.relu)
    ...
    def GetValues(self, input_images):
        values = self.sess.run(self.value, feed_dict={self.X: input_images})
        return values

    def SelectAction(self, input_images):
        values = self.sess.run(self.value, feed_dict={self.X: input_images})
    ...
    def Train(self, prev_images, choice, reward):
